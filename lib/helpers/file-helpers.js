...
export async function getBlogPostList() {
  const allDirs = await readDirectory('/content');
  const yearDirs = allDirs.filter(d => /^\d{4}$/.test(d));
  ...
  async function loadBlogPost(slug) {
    ...
    for (const yearDir of yearDirs) {
      for (const ext of ['.md', '.mdx']) {
        try {
          const rawContent = await readFile(`/content/${yearDir}/${slug}${ext}`);
          const { data: frontmatter, content } = matter(rawContent);
          if (frontmatter.draft) throw new Error(`Post not found: ${slug}`);
